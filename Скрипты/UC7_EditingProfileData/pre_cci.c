# 1 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c"
...
# 1 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c" 2
...
# 2 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c" 2
...
# 3 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c" 2
...
# 4 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c" 2
...
Action()
{
    ...
    lr_start_transaction("UC7_EditingProfileData");
    ...
    web_custom_request("AccountUpdateRequest",
        ...
        "Mode=HTML",
        "EncType=text/xml; charset=UTF-8",
        "Body=<?xml version=\"1.0\" encoding=\"UTF-8\"?><soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"><soap:Body><AccountUpdateRequest xmlns=\"com.advantage.online.store.accountservice\"><lastName>{lastName}</lastName><firstName>{firstName}</firstName><accountId>{userId}</accountId><countryId>{FullCountryName},{shortCountry}</countryId><stateProvince>{stateProvinceRegion}</stateProvince><cityName>{city}</cityName><address>{adress}</address><zipcode>{newPostalCode}</zipcode><phoneNumber>{newPhoneNumber}</phoneNumber><email>{email}</email><accountType>10</accountType><allowOffersPromotion>true</allowOffersPromotion><base64Token>Basic {lpBase64}</base64Token></AccountUpdateRequest></soap:Body></soap:Envelope>",
        "LAST");

    web_add_header("SOAPAction", "com.advantage.online.store.accountserviceGetAccountByIdNewRequest");

    web_reg_find("Text=\<ns2:mobilePhone\>{newPhoneNumber}\<\/ns2:mobilePhone\>","LAST");
    web_reg_find("Text=\<ns2:zipcode\>{newPostalCode}\<\/ns2:zipcode\>","LAST");
    web_reg_find("Text=\<ns2:id\>{userId}\<\/ns2:id\>","LAST");
    ...
    lr_end_transaction("Entry_AccountDetailsData",2);
    ...
    lr_end_transaction("UC7_EditingProfileData",2);

    return 0;
}
# 5 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c" 2
...
# 6 "c:\\users\\user\\documents\\vugen\\scripts\\uc7_editingprofiledata\\\\combined_UC7_EditingProfileData.c" 2
